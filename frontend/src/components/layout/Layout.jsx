...
const NAV_ITEMS = [
  ...
  { path: '/system/transfer', icon: '🔄', label: '离职移交' },
  ...
  return (
    <div className="flex h-screen overflow-hidden bg-gray-50">
      <aside className={`sidebar-bg flex-shrink-0 flex flex-col transition-all duration-300 ${sidebarCollapsed?'w-16':'w-56'}`}>
        ...
        <nav className="flex-1 overflow-y-auto py-3 px-2 space-y-0.5">
          {NAV_ITEMS.map((item, i) =>
            item.divider ? <div key={i} className="my-2 border-t border-white/10" /> : (
        ...
        <div className="p-2 border-t border-white/10 flex-shrink-0">
          <button onClick={toggleSidebar} className="w-full flex items-center justify-center gap-2 px-3 py-2 text-slate-300 hover:text-white hover:bg-white/10 rounded-lg transition-colors text-sm">
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        <header className="h-16 bg-white border-b border-gray-200 flex items-center justify-between px-6 flex-shrink-0 shadow-sm">
          <div className="flex items-center gap-4">
            <h1 className="text-gray-500 text-sm">客户关系管理系统 <span className="text-gray-300">v2.0</span></h1>
          </div>
          <div className="flex items-center gap-3">
            <div className="text-xs text-gray-400 hidden sm:block">
              <span className="badge-blue">{user?.role_name || '—'}</span>
            </div>
            <div className="relative">
              <button onClick={()=>setShowUserMenu(!showUserMenu)} className="flex items-center gap-2 px-3 py-1.5 rounded-lg hover:bg-gray-50 transition-colors">
              ...
              {showUserMenu && (
                <div className="absolute right-0 top-full mt-1 w-44 bg-white rounded-xl shadow-lg border border-gray-100 py-1 z-50 animate-scale-in">
                  <button onClick={()=>{ navigate('/profile'); setShowUserMenu(false) }} className="w-full text-left px-4 py-2.5 text-sm text-gray-700 hover:bg-gray-50 flex items-center gap-2"><span>👤</span> 个人中心</button>
                  <div className="border-t border-gray-100 my-1" />
                  <button onClick={handleLogout} className="w-full text-left px-4 py-2.5 text-sm text-red-600 hover:bg-red-50 flex items-center gap-2"><span>🚪</span> 退出登录</button>
                </div>
              )}
            </div>
          </div>
      ...
      <div className="fixed top-4 right-4 z-[100] space-y-2">
        {notifications.map(n=>(
          <div key={n.id} className={`animate-fade-in flex items-start gap-3 px-4 py-3 rounded-xl shadow-lg min-w-64 max-w-sm border ${n.type==='success'?'bg-emerald-50 text-emerald-800 border-emerald-200':n.type==='error'?'bg-red-50 text-red-800 border-red-200':'bg-blue-50 text-blue-800 border-blue-200'}`}>
            <span className="text-base flex-shrink-0">{n.type==='success'?'✅':n.type==='error'?'❌':'ℹ️'}</span>
