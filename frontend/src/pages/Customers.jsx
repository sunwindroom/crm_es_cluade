...
const INIT_FORM = { name:'', type:'company', industry:'', level:'normal', status:'active', region:'', address:'', website:'', phone:'', email:'', contact_name:'', contact_phone:'', contact_email:'', source:'', remark:'' }
const INIT_FOLLOWUP = { content: '', followup_type: 'call', followup_time: '', next_followup_time: '' }

const FOLLOWUP_TYPES = [{ value: 'call', label: '电话' }, { value: 'visit', label: '拜访' }, { value: 'email', label: '邮件' }, { value: 'wechat', label: '微信' }, { value: 'meeting', label: '会议' }]
...
export default function Customers() {
  ...
  const [followupModal, setFollowupModal] = useState({ open: false, customerId: null })
  const [followupForm, setFollowupForm] = useState(INIT_FOLLOWUP)
  ...
  const handleFollowup = async () => {
    if (!followupForm.content) return notify('error', '跟进内容不能为空')
    try {
      await customersAPI.addFollowup(followupModal.customerId, followupForm)
      notify('success', '跟进记录已添加')
      setFollowupModal({ open: false, customerId: null })
      setFollowupForm(INIT_FOLLOWUP)
    } catch (e) { notify('error', e?.message || '操作失败') }
  }
  ...
  const columns = [
    { key: 'name', title: '客户名称', render: (v, r) => (
    ...
    { key: 'created_at', title: '创建时间', render: v => fmt.date(v) },
    {
      ...
      render: (_, r) => (
        <div className="flex items-center gap-1">
          ...
          <button className="btn btn-sm text-emerald-600 hover:bg-emerald-50 border-0 shadow-none" onClick={() => { setFollowupModal({ open: true, customerId: r.id }); setFollowupForm(INIT_FOLLOWUP) }}>跟进</button>
  ...
  return (
    <div className="animate-fade-in">
      <PageHeader title="客户管理" desc="管理客户信息，跟踪客户动态，维护客户关系"
      ...
      <Modal open={modal.open} onClose={() => setModal({ ...modal, open: false })}
        title={modal.mode === 'create' ? '新建客户' : '编辑客户'} width="max-w-3xl"
        footer={<>
      ...
      {/* Detail Modal */}
      {d && (
        <Modal open={detailModal.open} onClose={() => setDetailModal({ open: false, data: null })}
          title={<span className="flex items-center gap-2">{d.name} <Badge status={d.level} /></span>}
          width="max-w-3xl"
          footer={<>
            <button className="btn-secondary" onClick={() => setDetailModal({ open: false, data: null })}>关闭</button>
            <button className="btn-primary" onClick={() => { setDetailModal({ open: false, data: null }); openEdit(d) }}>编辑</button>
          </>}
        >
          <Tabs active={detailTab} onChange={setDetailTab} items={[
            { key: 'info', label: '基本信息' },
            { key: 'contacts', label: '联系人', count: d.contacts?.length },
            { key: 'followups', label: '跟进记录', count: d.followups?.length },
          ]} />
          {detailTab === 'info' && (
            <div className="grid grid-cols-2 gap-x-8 gap-y-3 text-sm">
              {[
                ['客户名称', d.name], ['客户类型', d.type === 'company' ? '企业' : '个人'],
                ['所属行业', d.industry], ['客户级别', <Badge status={d.level} />],
                ['所在地区', d.region], ['客户来源', d.source],
                ['联系电话', d.phone], ['邮箱', d.email],
                ['主要联系人', d.contact_name], ['联系人电话', d.contact_phone],
                ['网站', d.website], ['负责人', d.owner_name],
              ].map(([l, v]) => (
                <div key={l} className="flex gap-2">
                  <span className="text-gray-400 flex-shrink-0 w-20">{l}</span>
          ...
          {detailTab === 'contacts' && (
            <div className="space-y-3">
              {d.contacts?.length === 0 && <div className="text-center py-8 text-gray-400">暂无联系人</div>}
              {d.contacts?.map(c => (
          ...
          {detailTab === 'followups' && (
            <div className="space-y-3">
              {d.followups?.length === 0 && <div className="text-center py-8 text-gray-400">暂无跟进记录</div>}
              {d.followups?.map(f => (
      ...
      {/* Followup Modal */}
      <Modal open={followupModal.open} onClose={() => setFollowupModal({ open: false, customerId: null })}
        title="添加跟进记录" width="max-w-lg"
        footer={<>
          <button className="btn-secondary" onClick={() => setFollowupModal({ open: false, customerId: null })}>取消</button>
          <button className="btn-primary" onClick={handleFollowup}>保存</button>
        </>}
      >
        <div className="space-y-4">
          <FormField label="跟进方式">
            <select className="form-select" value={followupForm.followup_type} onChange={e => setFollowupForm(f => ({ ...f, followup_type: e.target.value }))}>
              {FOLLOWUP_TYPES.map(t => <option key={t.value} value={t.value}>{t.label}</option>)}
            </select>
          </FormField>
          <FormField label="跟进时间">
            <input type="datetime-local" className="form-input" value={followupForm.followup_time} onChange={e => setFollowupForm(f => ({ ...f, followup_time: e.target.value }))} />
          </FormField>
          <FormField label="跟进内容" required>
            <textarea className="form-textarea" rows={4} value={followupForm.content} onChange={e => setFollowupForm(f => ({ ...f, content: e.target.value }))} placeholder="详细描述本次跟进情况..." />
          </FormField>
          <FormField label="下次跟进时间">
            <input type="date" className="form-input" value={followupForm.next_followup_time} onChange={e => setFollowupForm(f => ({ ...f, next_followup_time: e.target.value }))} />
          </FormField>
        </div>
      </Modal>

      <ConfirmDialog open={delConfirm.open} onClose={() => setDelConfirm({ open: false, id: null })}
        onConfirm={handleDelete} title="删除客户" message="确定要删除此客户吗？相关跟进记录也将一并删除。" />
    </div>
  )
}
