...
export function UsersManagement() {
  ...
  const [form,setForm]=useState({username:'',password:'',real_name:'',email:'',phone:'',role_id:'',department:'',position:'',status:1,manager_id:''})
  ...
  const load=useCallback(async()=>{
    setLoading(true)
    try{ const res=await systemAPI.listUsers({page,pageSize:PAGE_SIZE,...filters}); setList(res.data.list); setTotal(res.data.total) }
    finally{setLoading(false)}
  },[page,filters])

  useEffect(()=>{load()},[load])
  useEffect(()=>{
    systemAPI.listRoles().then(r=>setRoles(r.data))
    systemAPI.listUsers({pageSize:200}).then(r=>setAllUsers(r.data.list))
  },[])

  const openCreate=()=>{setForm({username:'',password:'',real_name:'',email:'',phone:'',role_id:'',department:'',position:'',status:1,manager_id:''});setModal({open:true,mode:'create',data:null})}
  ...
  const columns=[
    ...
    {key:'manager_name',title:'上级',render:v=>v||'—'},
    {key:'status',title:'状态',render:(v,r)=><span>{v?<span className="badge-green">启用</span>:<span className="badge-red">禁用</span>}{r.resigned?<span className="badge-red ml-1">离职</span>:''}</span>},
    {key:'last_login',title:'最后登录',render:v=>fmt.datetime(v)},
    {key:'actions',title:'操作',width:'180px',render:(_,r)=>(
  ...
  return (
    <div className="animate-fade-in">
      <PageHeader title="用户管理" desc="管理系统用户、角色分配和上下级关系"
      ...
      <div className="filter-bar shadow-sm">
        ...
        <select className="form-select w-32" value={filters.role_id} onChange={e=>setFilter('role_id',e.target.value)}>
          <option value="">全部角色</option>{roles.map(r=><option key={r.id} value={r.id}>{r.name}</option>)}
        </select>
      ...
      <Modal open={modal.open} onClose={()=>setModal({...modal,open:false})} title={modal.mode==='create'?'新建用户':'编辑用户'} width="max-w-2xl"
        ...
        <div className="grid grid-cols-2 gap-4">
          ...
          <FormField label="角色"><select className="form-select" value={form.role_id||''} onChange={e=>set('role_id',e.target.value)}><option value="">请选择角色</option>{roles.map(r=><option key={r.id} value={r.id}>{r.name}</option>)}</select></FormField>
          <FormField label="上级"><select className="form-select" value={form.manager_id||''} onChange={e=>set('manager_id',e.target.value)}><option value="">无（顶级）</option>{allUsers.filter(u=>u.id!==(form.id||modal.data?.id)).map(u=><option key={u.id} value={u.id}>{u.real_name||u.username} ({u.role_name||'—'})</option>)}</select></FormField>
          ...
          {modal.mode==='edit'&&<FormField label="状态"><select className="form-select" value={form.status} onChange={e=>set('status',parseInt(e.target.value))}><option value={1}>启用</option><option value={0}>禁用</option></select></FormField>}
        </div>
      </Modal>

      <Modal open={resetModal.open} onClose={()=>setResetModal({open:false,id:null})} title="重置密码" width="max-w-sm"
        footer={<><button className="btn-secondary" onClick={()=>setResetModal({open:false,id:null})}>取消</button><button className="btn-primary" onClick={handleResetPwd}>确认重置</button></>}>
        <FormField label="新密码" required hint="密码长度至少6位"><input type="password" className="form-input" value={newPwd} onChange={e=>setNewPwd(e.target.value)} placeholder="请输入新密码"/></FormField>
      </Modal>
      <ConfirmDialog open={delConfirm.open} onClose={()=>setDelConfirm({open:false,id:null})} onConfirm={handleDelete} title="删除用户" message="确定要删除此用户账号吗？"/>
    </div>
  )
}

const ALL_PERMS = [{module:'leads',label:'线索管理'},{module:'customers',label:'客户管理'},{module:'opportunities',label:'商机管理'},{module:'projects',label:'项目管理'},{module:'contracts',label:'合同管理'},{module:'payments',label:'回款管理'},{module:'reports',label:'报表统计'},{module:'system',label:'系统管理'}]
const PERM_ACTIONS = [{value:'read',label:'查看'},{value:'write',label:'编辑'},{value:'delete',label:'删除'}]

export function RolesManagement() {
  const [roles,setRoles]=useState([]); const [loading,setLoading]=useState(false)
  const [modal,setModal]=useState({open:false,mode:'create',data:null})
  ...
  const load=async()=>{ setLoading(true); try{const res=await systemAPI.listRoles();setRoles(res.data)}finally{setLoading(false)} }
  useEffect(()=>{load()},[])
  ...
  const togglePerm=(module,action)=>{const perms={...form.permissions};if(!perms[module])perms[module]=[];if(perms[module].includes(action)){perms[module]=perms[module].filter(a=>a!==action)}else{perms[module]=[...perms[module],action]};setForm(f=>({...f,permissions:perms}))}
  const hasAllModule=(module)=>PERM_ACTIONS.every(a=>(form.permissions?.[module]||[]).includes(a.value))
  const toggleAllModule=(module)=>{const perms={...form.permissions};if(hasAllModule(module)){perms[module]=[]}else{perms[module]=PERM_ACTIONS.map(a=>a.value)};setForm(f=>({...f,permissions:perms}))}

  return (
    <div className="animate-fade-in">
      <PageHeader title="角色管理" desc="配置系统角色和权限，实现精细化访问控制" actions={<button className="btn-primary" onClick={openCreate}>＋ 新建角色</button>}/>
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        {loading?<div className="col-span-3 text-center py-16 text-gray-400">加载中...</div>:roles.map(role=>(
          <div key={role.id} className="card p-5 hover:shadow-md transition-shadow">
            <div className="flex items-start justify-between mb-3">
              <div><h3 className="font-semibold text-gray-800">{role.name}</h3><p className="text-xs text-gray-500 mt-0.5">{role.description}</p></div>
      ...
      <Modal open={modal.open} onClose={()=>setModal({...modal,open:false})} title={modal.mode==='create'?'新建角色':'编辑角色'} width="max-w-2xl"
        footer={<><button className="btn-secondary" onClick={()=>setModal({...modal,open:false})}>取消</button><button className="btn-primary" onClick={handleSave} disabled={saving}>{saving?'保存中...':'保存'}</button></>}>
            ...
            <div className="border border-gray-200 rounded-lg overflow-hidden">
              ...
              {ALL_PERMS.map(perm=>(
                <div key={perm.module} className="grid grid-cols-5 gap-0 px-4 py-3 border-b border-gray-100 last:border-0 hover:bg-gray-50">
                  ...
                  <div className="flex justify-center"><input type="checkbox" checked={hasAllModule(perm.module)} onChange={()=>toggleAllModule(perm.module)} className="rounded"/></div>
                  {PERM_ACTIONS.map(action=>(
                    <div key={action.value} className="flex justify-center"><input type="checkbox" checked={(form.permissions?.[perm.module]||[]).includes(action.value)} onChange={()=>togglePerm(perm.module,action.value)} className="rounded"/></div>
                  ))}
                </div>
              ))}
            </div>
      ...
      <ConfirmDialog open={delConfirm.open} onClose={()=>setDelConfirm({open:false,id:null})} onConfirm={handleDelete} title="删除角色" message="确定要删除此角色吗？请确保无用户使用此角色。"/>
    </div>
  )
}
